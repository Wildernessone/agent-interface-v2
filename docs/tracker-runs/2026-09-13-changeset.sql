-- Tracker changeset for the weekly refresh of 2026-09-13.
-- Generated from 2026-09-13-changeset.json; the JSON is the source, this file is the vehicle.
-- Per CLAUDE.md the tracker is updated by ONE row in tracker_changesets (v2 Supabase
-- oqbpuspnmznqxgkmyzyb), not by a file under functions/_tracker/. It lands as status='draft'
-- and the hourly ai-tracker-auto-publish cron flips it to 'published' 48h later unless it is
-- set 'archived' first. Running this statement is the human decision that starts that clock.
-- Executed 2026-09-14 14:12 UTC by the session that merged #152 (row weekly-refresh-2026-09-13, status draft,
-- auto_publish_at 2026-09-16 14:12 UTC). The note below is the text that ran.
insert into public.tracker_changesets (checked, slug, entries, note)
values (
  '2026-09-13',
  'weekly-refresh-2026-09-13',
  $j$[
  {
    "id": "webmcp",
  ...
  {
    "id": "agents-md",
    "call": "Boring, tiny, and already everywhere, the kind of standard that wins by being trivial to adopt. agents.md puts it at \"over 60k open-source projects\" (read 2026-09-13) and the format repo is still moving (last push 2026-09-10). Add one to every repo you own.",
    "name": "AGENTS.md",
    "group": "protocol",
    "links": [
      [
    ...
    "short": "A plain-markdown convention: one file in a repo telling coding agents how to work in that codebase. The project site counts over 60,000 open-source projects carrying one; the format is stewarded by the Agentic AI Foundation.",
    "status": "rising",
  ...
  },
  {
    ...
    "links": [
      ...
      [
        "OpenAI docs index (llms.txt)",
        "https://developers.openai.com/llms.txt"
      ]
    ],
    "short": "Apps that render inside ChatGPT and act through Codex. The standalone App Directory (Dec 2025) was folded into a shared Plugin Directory on Jul 9, 2026, and developers.openai.com/apps-sdk now 301-redirects to /plugins, though OpenAI’s own docs index still titles the section “ChatGPT plugins and Apps SDK”: the URL is retired, the name is not quite.",
    "status": "rising",
    "steward": "OpenAI (built on MCP; distributed through OpenAI’s own plugin format)",
    "statusLabel": "Rising"
  },
  {
    "id": "zed-acp",
    "call": "The standard seam between IDEs and coding agents, and no longer one vendor’s project — governance is shared with JetBrains under an interim model with two lead maintainers, Ben Brandt (Zed) and Sergey Ignatov (JetBrains, announced 2026-02-19), and “transitioning to an independent foundation” as the stated goal. We had this as a Zed project through 2026-08-10; that was already wrong when we wrote it. Adoption is the real story: 42 agent directories in the registry, Cursor among them, and Zed 1.0 made ACP its headline capability. A v2 draft is still a draft: schema-v2.0.0-alpha.3 (2026-08-20) is the newest v2 artifact, no release of any kind has followed it as of 2026-09-13, and v1 shipped alongside it that same day, so build on v1. Terminology trap: unqualified “ACP” now usually means this, not IBM’s dead protocol of the same acronym.",
    "name": "ACP — Agent Client Protocol (Zed)",
    "group": "protocol",
    "links": [
      [
        "Spec",
    ...
    "short": "The \"LSP for coding agents\": a JSON-RPC protocol connecting any editor to any coding agent. The joint agent registry now lists 42 agents, including Cursor, Devin, GitHub Copilot, Gemini and Codex.",
    "status": "live",
    "steward": "Jointly governed by Zed Industries and JetBrains under an interim model that names an independent foundation as the eventual home",
  ...
  },
  {
    ...
    "links": [
      ...
      [
        ...
        "https://github.com/agentplugins/agent-plugins-spec/blob/main/spec/1.1.0.md"
      ],
      [
        "Governance",
      ...
      [
        ...
        "https://github.com/agentplugins/agent-plugins-spec/pull/37"
      ]
    ],
    "short": "A shared package format for bundling Agent Skills and MCP servers into one directory — plugin.json, skills/, mcp.json — readable by ChatGPT, Codex, Cursor, GitHub Copilot, Kiro and VS Code. v1.0.0 shipped Aug 6, 2026. A 1.1.0 file sits in the spec repo marked Working Draft.",
    "status": "early",
    "steward": "Technical Steering Committee (no foundation): five maintainers from Amazon, Cursor/Anysphere, Microsoft, OpenAI and Vercel; proposed by Vercel",
    "statusLabel": "Early"
  },
  {
    "id": "agent-builder",
    "call": "Deprecation notice eight months after launch: the visual builder and evals shut down 2026-11-30, with existing evals going read-only on 2026-10-31. We said OpenAI points builders back to code alone; that is half the sentence. The deprecation page names two paths, and the second is Workspace Agents — a research preview on Business, Enterprise, Edu and Teachers plans, powered by Codex, which OpenAI positions as the successor to GPTs. So the replacement for visual agent-building is another managed surface, not just the SDK. Notably, eval users are handed to a third-party open-source tool rather than an OpenAI product. ChatKit survives.",
    "name": "OpenAI AgentKit (Agent Builder & Evals)",
    "group": "graveyard",
    "links": [
      [
        "Deprecations page",
  ...
  },
  {
    ...
    "name": "Computer use (screen-level control)",
    "group": "protocol",
    "links": [
      [
      ...
      [
        ...
        "https://platform.claude.com/docs/en/release-notes/api.md"
      ],
      [
        "OpenAI computer use",
        "https://learn.chatgpt.com/docs/computer-use.md"
      ],
      [
        "Gemini computer use",
    ...
    "short": "The protocol-less path: the agent operates the same screen, cursor and keyboard a human would. Anthropic’s tool went stable on 2026-08-19 after nearly two years in beta; OpenAI runs it in ChatGPT Work and Codex on macOS and Windows; Google’s Gemini computer use covers browser, mobile and desktop.",
    "status": "live",
  ...
  },
  {
    ...
    "name": "ACP — Agent Communication Protocol (IBM)",
    "group": "graveyard",
    "links": [
      [
        "LF announcement",
  ...
  $n$Weekly refresh 2026-09-13, run by hand against primary sources, then re-verified by an independent refuter and corrected 2026-09-14 (ag-ui event counts; the mcp Tier 1 SDK sentence and the deprecations; the openai-apps package format; the ap2 partner figure now cites Google's launch post; the WebKit label date). Scope: 16 of 29 live entries checked (14 fully and restated here; a2a and atlas only partially, so not restated); 13 not checked this week keep their 2026-08-26/27 dates. MOVED: webmcp (WebKit standards-positions #670: opposition stated 2026-06-03, label position: oppose 2026-06-11; Mozilla #1412 labeled position: neutral and closed 2026-08-05; Draft CG Report now dated 10 September 2026; implementation-status.md lists ChatGPT Desktop (2026-08-26) and Brave Leo; README headless scope PR #296 merged 2026-09-04; chromestatus.com/feature/5117755740913664 still OT 149-156 with TAG/security/privacy Pending; Edge trial expiration November 17, 2026 confirmed), x402 (x402.org/members: 52 organizations = 17 Premier + 24 General + 11 Associate; homepage counter 75.41M tx / $24.24M 30-day), agents-md (agents.md: "used by over 60k open-source projects", "now stewarded by the Agentic AI Foundation under the Linux Foundation"; repo pushed 2026-09-10), openai-apps (developers.openai.com/apps-sdk 301 -> /plugins; plugins.md now documents a portable root plugin.json with .codex-plugin as a compatibility fallback; published plugins still .codex-plugin only), zed-acp (agentclientprotocol.com/community/governance: "ACP has two lead maintainers: Ben Brandt (Zed Industries) and Sergey Ignatov (JetBrains)", governance.mdx commit 2026-02-19; registry 42 agent dirs; no release after 2026-08-20), mcp (registry figure dated, count not re-counted; current revision 2026-07-28; four Tier 1 SDKs per the release post, Rust in beta outside the set; Roots/Sampling/Logging, Dynamic Client Registration and the legacy SSE transport deprecated), ag-ui (docs.ag-ui.com/concepts/events: 37 documented event types incl. 1 draft type and 5 deprecated, 31 current; README names no foundation; release 2026-09-11). UNCHANGED: agent-plugins (PR #37 open, MAINTAINERS.md five names, last commit 2026-08-19), agent-builder (developers.openai.com/api/docs/deprecations: Agent Builder and Evals shut down 2026-11-30, evals read-only 2026-10-31), computer-use (platform.claude.com: computer_toolset_20260801 on Claude API and Google Cloud; learn.chatgpt.com: ChatGPT Work and Codex on macOS and Windows, Locked use plug-in; ai.google.dev: browser/mobile/desktop, updated 2026-09-04), agent-protocol (repo last commit 2025-04-08; agentprotocol.ai is an independent guide), ibm-acp (archived 2025-08-27), operator (operator.chatgpt.com 308 -> chatgpt.com/?system_hints=agent), ap2 (v0.2.0 2026-04-28; FIDO 2026-04-28 post names Mastercard and Visa chairs; Google's 2025-09-16 launch post: "more than 60 organizations"). NOT re-verified: a2a stage/board claims (aaif.io renders them client-side; v1.0.1 2026-05-28 still newest, v1.1 milestone due 2026-10-22 confirmed), atlas post-8/9 shutdown (openai.com and help.openai.com 403 automated fetch). Full report: docs/tracker-runs/2026-09-13-weekly-refresh.md in the repo.$n$
);
