...
export function buildSystemPrompt({ activeAgentIds=[], enabledTools={}, mode="concise", round=1, totalRounds=1, agentId="claude", voiceMode=false, memoryContext="", projectContext="", projectName="", projectBrief="", leadAgent=null, role=null, skills=null, useSkills=true, buildDebate=false }) {
  ...
  lines.push(`\nYou produce TEXT ONLY. You do NOT run tools and you have no "image tool", "my generator", or any tool of your own — the BUILD pipeline (not you) makes every file, image, flyer, audio, and video. So NEVER perform, simulate, or PROMISE to perform an action yourself: do not say "I will generate/create/make a [flyer/image/doc]", "I'll use my image generation tool", "Specific Output: I will produce…", or claim you already did something (sent an email, saved a file, made a deck). Those are hollow promises — nothing gets made, and the user is left waiting. Instead, if a concrete deliverable would genuinely help, OFFER it as something the panel can build — e.g. "Want a flyer for that? Say the word and the panel will build it." — so the user can trigger a real build. Never fake doing it, and never say you're unable (the build system can).`)
  lines.push([
